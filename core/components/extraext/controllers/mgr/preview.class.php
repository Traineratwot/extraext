	...
	if (class_exists('extraExtManagerController')) {
		...
		class ExtraextMgrPreviewManagerController extends extraExtManagerController
		{
			...
			public function loadCustomCssJs()
			{
				...
				if (in_array($this->type,['video','audio','image','bin'])) {
					if (ob_get_level()) {
						ob_end_clean();
					}
					...
					$content = htmlspecialchars(file_get_contents($this->path));
					$this->addHtml('<title>' . $this->name . '</title>');
					$this->addHtml('<pre><code  class="extraExt language-' . $this->ext2lang($this->ext) . '">' . $content . '</code></pre>');
					$this->addHtml("
					<script>
