	...
	class extraExtManagerController extends modExtraManagerController
	{
		...
		public function __construct(modX &$modx, $config = [])
		{
			...
			try {
				...
				$this->addCss('js/highlight/styles/github.css', $this->extraExtUrl);
				$this->addCss('css/main.tab.css', $this->extraExtUrl);
				$this->addJavascript('js/highlight/highlight.pack.js', $this->extraExtUrl);
			...
			if ($cache and $remote) {
				try {
					...
					if (is_array($this->cachePaths) and array_key_exists($hash, $this->cachePaths)) {
						...
						$tmp = "components/extraext/js/cache/" . $hash . '.' . $ext;
